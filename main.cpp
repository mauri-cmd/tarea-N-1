...
vector<string> separarterminos(string polinomio) {//separa cada termino y lo pone en un vector
    ...
    for (int i = 0; i < polinomio.length(); i++) {
        if (polinomio[i] == '+'||polinomio[i] == '-') { // pregunta si es que hya un + o -
            ...
            termino += polinomio[i];
            if (i < polinomio.length() - 2 && polinomio[i+1] == '*' && polinomio[i+2] == '*') {
                termino += "**";
                i += 2;
            }
        } else if (polinomio[i] == '*') {
...
std::reverse(coeficiente.begin(), coeficiente.end());
...
int m=0;
    ...
    for(int j = 0; j < coeficienteordenado.size(); j++){
    ...
    if(coeficienteordenado.size()>gradord.size()){
...
vector<double> raicesPolinomioGrado3(string polinomio) { // calcular raices de polinomio de tercer grado
    ...
    if (n != 4 || grados[0] != 3) {
        cout << "El polinomio no es de grado 3" << endl;
        return {};
    }
...
vector<double> coeficientesordenados = ordencoef(coeficientesord,todosgrados,gmax);
  ...
  for(int i = 0 ;i < coeficientesordenados.size();i++){
    cout << "coeeficiente " << coeficientesordenados[i] << endl;
  }
